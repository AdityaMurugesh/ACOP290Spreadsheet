...
Our test suite contains 59 automated tests that verify correct behavior across the following categories:
...
\midrule
Basic operations & Set constant (\texttt{A1=2}), cell reference (\texttt{B1=A1}), arithmetic (\texttt{A1+3}), integer division truncation (\texttt{5/2=2}), multiplication, subtraction to zero, negative constants and results \\
Functions & MIN, MAX, AVG, SUM, STDEV with ranges; SLEEP returns its argument value; SLEEP(0); SLEEP with negative value returns ERR \\
Ranges & 1D range (\texttt{A1:C1}), 2D range (\texttt{A1:B2}), single-cell range, invalid range where start $>$ end, out-of-bounds range \\
Error handling & Division by zero produces ERR; ERR propagates through dependent cells; ERR in range makes function ERR; ERR recovery after source is corrected \\
Dependencies & Single recalculation (\texttt{B1=A1+1}, change A1); chained recalculation across 3 cells; multiple cells depending on the same source; overwrite formula with constant removes dependency \\
Circular deps & Self-reference (\texttt{A1=A1+1}); two-cell and three-cell cycles; circular via range; recovery after rejection; old formula preserved after circular rejection \\
Boundary checks & Out-of-bounds target cell (\texttt{A3=5} on 2$\times$2 sheet); out-of-bounds cell reference in formula; invalid program arguments (\texttt{./sheet 0 0}) \\
Helper commands & Unrecognized command shows error; \texttt{disable\_output} suppresses sheet display; \texttt{enable\_output} restores display; \texttt{scroll\_to} with valid and out-of-bounds cells; scroll at boundaries (w/a at top-left) \\
Input handling & Empty formula (\texttt{A1=}); spaces-only input; formula with spaces (\texttt{A1 = 2 + 3}) \\
\bottomrule
\end{tabular}
\end{table}

\noindent Tests are run with \texttt{make test}, which compiles \texttt{tests/test.c} and executes it. Each test launches \texttt{./sheet} with piped input and checks the output for expected strings.
